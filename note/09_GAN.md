...
<img src="../image/10/GAN_concept.PNG" width=80%>
...
여기서 GAN이 중요한것은 Generative Model이다.
...
<img src="../image/10/GAN_architecture.PNG" width=80%>
...
- $\theta_d$ : Discriminative, $\theta_g$ : Generative
  - x가 real일 때 : Loss = $\log{D_{\theta d}(x)}$ → Max
  - x가 Fake일 때 : Loss = $1 - \log{D_{\theta d} (G_{\theta g}(z))}$ → Min
-  $G_{\theta g}(z)$ 에서 z는 Generative model의 latent vector, $D_{\theta d}(G_{\theta g}(z))$ 는 $G_{\theta g}(z)$가 real에 가까울수록 $D_{\theta d}(G_{\theta g}(z)) \approx 1$. **즉, Discriminative가 real이라고 판단하게끔 하는것이 목표다.** 따라서 $1 - \log{D_{\theta d} (G_{\theta g}(z))}$ 을 1 ~ ∞ 사잇값에서 _**1로 수렴하게 하는것이 필요하다.**_
...
<img src="../image/10/saddle.png" width=80%>
...
> _**GAN의 단점이 optimize 하기가 매우 어렵다.**_
...
4. 만약 discriminator가 real data와 같아진다면 종료한다. 이 때 $D^*(x) = \frac{P_{data}(x)}{P_{data}(x) + P_{g}(x)}$ 는 real data일 확률과 Generative model의 data일 확률이 같은 지점, Discriminator가 구분하지 못하는 지점이므로 종료한다.
...
<img src="../image/10/DCGAN_upsampling.PNG" width=80%>
...
2. BatchNorm을 Discriminator, Generator 둘다 씀, Generator의 output, Discriminator의 input layer 에는 넣지 않았다.
3. Fully connected layer를 Deep한 구조를 위해 피함 : Global Average Pooling을 이용
4. Generator 모든 layer들은 ReLU, 마지막 output은 Tanh(Batch 때문에)
5. Discriminator는 모든 layer들은 LeakyReLU (-구역은 0.2)

### 7-3. Example

<img src="../image/10/DCGAN_example.PNG" width=80%>
...
<img src="../image/10/GAN_ZOO.PNG" width=80%>
...
<img src="../image/10/GAN_Dissection.PNG" width=80%>

1. 학습된 generator를 이용해서 single unit u를 뽑아낸다. (그림에서 latent vector에서 뽑는데 틀렸당...)
2. unit u node를 뽑아내서 feature map을 만든다.
3. feature map은 upsampling을 통해 thresolded를 생성한다.
4. generated image을 segmentation을 통해 upsampling한 image와 IOU로 비교한다.

### 9-3. Purpose

<img src="../image/10/GAN_Dissection_purpose.PNG" width=80%>
<img src="../image/10/GAN_Dissection_purpose2.PNG" width=80%>
...
GAN을 학습하는 것은 대단히 어려운 일이다. 
1. Loss Function 자체가 Saddle Point 이기도 하고, 
2. Generator와 Discriminator 사이 학습속도도 서로 맞춰야한다.
...
<img src="../image/10/GAN_prior.PNG" width=80%>

실제로 GAN은 정말 중요하다. GAN으로 미리 generator를 만들어두면 여러가지 방면으로 쓰일 수 있다.
